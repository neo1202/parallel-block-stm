...
std::optional<Task> get_next_task(Scheduler& sched) {
    std::vector<Task> q;
    sched.next_task(q, 1);
    if (!q.empty()) {
        auto ret = q.back();
...
TEST_CASE("Scheduler: dependency and resume") {
    ...
    // T0 starts
    auto t0 = get_next_task(sched); // gets Exec(0)
    
    // T1 starts
    auto t1 = get_next_task(sched); // gets Exec(1)
